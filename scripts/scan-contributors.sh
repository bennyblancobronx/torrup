...
#   ./scan-contributors.sh [directory] --fix-all # AUTO-FIX everything (strip all issues)
...
should_skip_file() {
    ...
    # ONLY skip lock files (binary-ish, auto-generated)
    [[ "$file" == *package-lock.json ]] && return 0
    [[ "$file" == *yarn.lock ]] && return 0
    [[ "$file" == *Cargo.lock ]] && return 0

    # NO OTHER SKIPPING - we strip everything
    return 1
}
    ...
    while IFS= read -r file; do
        ...
        local changed=false
            ...
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' '/[Cc]o-[Aa]uthored-[Bb]y/d' "$file"
            else
                sed -i '/[Cc]o-[Aa]uthored-[Bb]y/d' "$file"
            fi
            changed=true
        ...
        if grep -qiE "generated (by|with)" "$file" 2>/dev/null; then
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' '/[Gg]enerated \(by\|with\)/d' "$file"
            else
                sed -i '/[Gg]enerated \(by\|with\)/d' "$file"
            fi
            changed=true
        ...
        # Strip author/contributor lines with external emails
            if ! grep -qiE "(author|contributor|written by|created by).*${ALLOWED_EMAIL}" "$file" 2>/dev/null; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                else
                fi
                changed=true
