...
from src.utils.qbittorrent import get_qbt_client, map_qbt_category_to_media_type
...
def _qbt_monitor_pass(conn, client, source_cats: list[str], category_map: str) -> int:
    """Run one qBitTorrent monitor pass and return count of queued items."""
    ...
    for tor in torrents:
        ...
        media_type = map_qbt_category_to_media_type(tor_cat, category_map)
        if not media_type:
            if "music" in tor_cat:
                media_type = "music"
            elif "movie" in tor_cat:
                media_type = "movies"
            elif "tv" in tor_cat:
                media_type = "tv"
            elif "book" in tor_cat:
                media_type = "books"
            else:
...
def qbt_monitor_once() -> int:
    """Run a single qBitTorrent monitor pass and return queued count."""
    with db() as conn:
        ...
        return _qbt_monitor_pass(conn, client, source_cats, category_map)
    ...
    while not shutdown_event.is_set():
        try:
            with db() as conn:
                ...
                added = _qbt_monitor_pass(conn, client, source_cats, category_map)
                if added:
                    logger.info(f"qBT Monitor: queued {added} item(s) this pass.")
